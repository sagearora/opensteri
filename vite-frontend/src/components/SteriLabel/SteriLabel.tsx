...
import { SteriLabelFragment } from "../../__generated__/graphql";
import { Loader2 } from "lucide-react";
import dayjs from "dayjs";
import { QRCodeSVG } from 'qrcode.react'
import { QRType, createQr } from "../../lib/qr-service";
import { Button } from "../ui/button";

export type SteriLabelProps = {
    item: SteriLabelFragment
    remove?: () => void;
    loading?: boolean;
}
...
function SteriLabel({
    ...
    return (
        <div className={`rounded-md p-2 relative overflow-hidden ${color}`}>
            ...
            {more_info && <>
                {item.steri_cycle_user && item.steri_cycle && <div className='mt-2'>
                    ...
                    <p className='text-sm'>Loaded By: {item.steri_cycle_user.name}</p>
